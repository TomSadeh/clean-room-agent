# [ARCHIVED] Clean Room Agent Meta-Plan
...
- Build the N-stage retrieval pipeline: stage protocol, pipeline runner, task analysis, budget enforcement, context assembly, and `cra retrieve`. Each retrieval stage performs deterministic pre-filtering followed by an LLM judgment call. MVP ships with Task Analysis + two retrieval stages (Scope, Precision) → Execute (4 prompts total); the architecture supports adding stages without structural changes.
...
|-------|-----------|--------|------------|
| Phase 1 | **Creates schema + populates** (indexing) | **Creates schema** + logs index run metadata + stores LLM enrichment outputs | **Defines schema only** (no per-task DB files created) |
| Phase 2 | Read-only (query API, enforced via read-only connection mode) | Append retrieval decisions + stage LLM call logs | **Creates** per-task, writes retrieval state |
...
- `cra enrich` is optional. Writes to raw DB (`enrichment_outputs` table); `--promote` copies to curated (`file_metadata`). Retrieval works without enrichment (Tier 4 skipped, stages use own LLM judgment). When enrichment IS present (promoted to curated), stages get richer signals and LLM calls are faster/cheaper.
- `cra retrieve` requires `--model` and `--base-url` (every retrieval stage uses LLM calls). Single mode (pipeline) — no `--mode` flag needed until a second mode exists.
...
- Config provides values that CLI flags can override. In development mode, if a required value is missing from BOTH config and CLI flags, it's a hard error. Config is not a silent fallback — it's a way to avoid retyping the same flags.
