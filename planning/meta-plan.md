...
- `cra enrich` is required before `cra solve`. Phase 2 scope expansion depends on `file_metadata`. Single mode (pipeline) — no `--mode` flag needed until a second mode exists.
...
`.clean_room/config.toml` stores project-level settings. Created by `cra init`. Precedence: CLI flags > config file values > hard error (no implicit defaults).
...
- `cra init` creates the config interactively or from explicit flags. Does not populate values the user doesn't provide.
- All `cra` subcommands (`index`, `enrich`, `retrieve`, `solve`) load config if it exists, then override with any CLI flags.
- Config loading is a thin utility (`config.py`), not a framework. Reads TOML, returns a flat dict, done.
- The `--budget-config` flag (Phase 2/3) is a separate mechanism for sharing budget values between tools — it does not replace `.clean_room/config.toml`.
